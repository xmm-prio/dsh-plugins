...
export declare const ENDPOINT_NAMESPACE = "session-archive";
/** Compose the wire endpoint name for one operation. */
export declare function endpointName(operation: keyof EndpointMap): string;
/** Compose the exact `/api` route path the host registers for one operation. */
export declare function endpointPath(operation: keyof EndpointMap): string;
/**
...
export type CapabilityReport = {
    readonly [K in CapabilityId]: CapabilityStatus;
};
...
export type FailureCode = 'capability-disabled' | 'invalid-session-id' | 'not-archived' | 'archive-set-unreadable' | 'unknown-session' | 'teardown-effect-missing' | 'write-lease-held' | 'log-root-unknown' | 'legacy-log-not-found' | 'log-path-refused' | OwnershipFailureCode | 'remove-failed'
...
export declare const TRANSPORT_FAILURE: {
    readonly badRequest: "session-archive/bad-request";
    readonly handlerFailed: "session-archive/handler-failed";
    readonly noConnection: "session-archive/no-connection";
    readonly transport: "session-archive/transport";
};
/** Request payload of every endpoint, keyed by operation. */
export interface EndpointMap {
    capabilities: {
        readonly request: Record<string, never>;
        readonly response: CapabilitiesResult;
    };
    list: {
        readonly request: Record<string, never>;
        readonly response: ArchiveListResult;
    };
    unarchive: {
        readonly request: {

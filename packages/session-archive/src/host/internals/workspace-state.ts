...
export type DomainRead =
  | { readonly ok: true; readonly state: WorkspaceDomainState }
  | { readonly ok: false; readonly reason: DomainReadFailure }

/** The private write handle, once the probe has confirmed it is there. */
interface PrivateRegistryShape {
...
/** Domain name of the workspace registry's storage. */
 ...
 */
export function readWorkspaceDomainState(facility: DomainFacilityLike | undefined): DomainRead {
  if (facility === undefined || typeof facility.get !== 'function') {
    return { ok: false, reason: 'domain-facility-unavailable' }
  }

  let handle: DomainHandleLike | undefined
  try {
    handle = facility.get(WORKSPACE_DOMAIN)
  } catch {
    return { ok: false, reason: 'domain-not-open' }
  }
  if (handle === undefined || typeof handle.global?.get !== 'function' || typeof handle.global.set !== 'function') {
    return { ok: false, reason: 'domain-not-open' }
  }
  ...
  const archived = (value as Record<string, unknown>)['archivedSessionIds']
  if (!Array.isArray(archived) || archived.some((id) => typeof id !== 'string')) {
    return { ok: false, reason: 'state-malformed' }
  }
  return { ok: true, state: value as WorkspaceDomainState }
}
...
): { readonly next: WorkspaceDomainState; readonly dropped: readonly string[] } {
 ...
 */
export function enqueueRegistryOperation<T>(
  registry: WorkspaceRegistryLike,
  operation: () => Promise<T>,
): Promise<T> {
  return (registry as unknown as PrivateRegistryShape).enqueueOperation(operation)
}
 ...
 */
export function backfillRegistryState(registry: WorkspaceRegistryLike, next: WorkspaceDomainState): void {
  ;(registry as unknown as PrivateRegistryShape).state = next
}
 ...
 */
export async function writeWorkspaceDomainState(
  facility: DomainFacilityLike,
  next: WorkspaceDomainState,
): Promise<void> {
  const handle = facility.get(WORKSPACE_DOMAIN)
  if (handle === undefined) throw new Error('session-archive: workspace storage domain closed mid-write')

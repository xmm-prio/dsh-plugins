...
export type ShutdownPlan =
  ...
  | { readonly kind: 'targets'; readonly targets: readonly ShutdownTarget[] }
...
export interface ShutdownCopy {
  ...
  report(closed: number, failures: readonly Extract<OperationOutcome, { ok: false }>[]): string
...
export function useShutdown(
  api: ArchiveApi,
  collect: () => Promise<ShutdownPlan>,
  copy: ShutdownCopy,
  onReport: (report: ShutdownReport) => void,
): ShutdownFlow {
  const [busy, setBusy] = useState(false)
  const [pending, setPending] = useState<readonly ShutdownTarget[] | undefined>(undefined)
  ...
  const start = useCallback(() => {
    ...
    void (async () => {
      ...
      if (plan.targets.length === 0) {
        onReport({ message: copy.nothing, ok: true })
        return
      }
      setPending(plan.targets)
    })()
  }, [busy, collect, copy, onReport])
  ...
  const confirm = useCallback(() => {
    ...
    void (async () => {
      ...
      setBusy(false)
      if (!outcome.ok) {
        onReport({ message: copy.transport(outcome), ok: false })
        return
      }
      ...
      onReport({
        message: copy.report(outcome.value.outcomes.length - failures.length, failures),
        ok: failures.length === 0,
      })
    })()
  }, [api, busy, copy, onReport, pending])

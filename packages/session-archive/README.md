...
- **关闭运行中会话**：释放会话在内存里占用的 agent。归档区里一次性关掉**后台**运行中的会话，用于收工；会话标题栏上关掉**这一个**会话。关闭只动内存，日志一字不改，会话再打开就会重新加载。
...
| 关闭会话 | cordis registry 上可扫描的 agent 生命周期 effect，以及 `ctx.agents.roots()` | 两个关闭入口都不可用 |
...
│   ├── archive-writer.ts
│   ├── agent-teardown.ts
│   ├── rejection-watch.ts   # 关闭期间给未捕获拒绝点名，不阻止宿主退出
│   ├── log-remover.ts
│   ├── metadata-reader.ts
...
└── client/
    ...
    ├── shutdown/            # 两个关闭入口共用的一条流程
    │   ├── useShutdown.ts   # 收集 → 确认 → 调用 → 汇报，两边不会走岔
    │   ├── targets.ts       # 谁可以被关：归档区排除当前会话的规则
    │   ├── useRunningAgent.ts   # 某个会话有没有 agent，判据来自宿主
    │   ├── ShutdownConfirmation.tsx  # 列名单的二次确认
    │   └── SessionShutdown.tsx       # 会话标题栏按钮
    ├── sidebar/             # 侧栏行内按钮
    │   ├── adapter.ts       # 唯一识别内置侧栏 DOM 与 fiber 的地方，带版本号
    │   ├── row-buttons.ts   # 注入、幂等重扫、kill-switch
    │   └── install.ts       # 观察器与生命周期
    ├── stylesheet.ts        # 注入一张样式表，各处样式共用
    └── text.ts              # 全部界面文案
```
...
**关闭不是删除。** 关闭释放的是会话在内存里的 agent——模型客户端、工具、日志写租约；会话日志一字不改，会话仍在侧栏里，再打开就会重新加载出一个新的 agent。删除针对日志，不可逆。两者唯一的共同点是都要先停掉 agent。

宿主没有 idle 回收也没有 LRU，agent 一旦 resume 就活到进程退出；而在 Web UI 里**打开一个会话就会 resume 它**。所以浏览过的会话会一直攒着，关闭就是把它们放掉。

底层只有一个通用原语 `shutdown(ids)`。「关闭全部」不是一个端点，而是调用方的策略：先 `running()` 拿到名单，再 `shutdown` 那批 id。因此二次确认列出的是**具体哪几个会话**，而不是一个没人核对得了的数字。

两个入口分工明确：

| 入口 | 关什么 | 为什么 |
| --- | --- | --- |
| 归档区底部按钮 | 后台运行中的会话 | 用途是释放后台占用。**浏览器当前打开的那个会话被排除**——前台正在用的不属于后台，它也是用户最不会预期被别的面板关掉的那个。 |
| 会话标题栏按钮 | 这一个会话 | 当前会话只能从这里关。关闭父会话时子代理随之释放，所以「关掉这个会话的所有 agent」就是这一个调用。 |

标题栏按钮占的是官方公开扩展点 `conversation.session.header.utilities`，组件从标准 props 拿到自己的 `sessionId`，**不涉及任何 DOM 改写**——和上面那条侧栏注入的路子完全不同。

按钮只在该会话确有 agent 时出现，判据来自宿主的 `running()`。这里**不能用会话列表的 `running` 位**：那个位说的是「有一轮对话在进行中」，而空闲的 agent 照样占着资源，那正是需要关闭的情形。探测失败时按钮保持原状而不是消失——连接抖一下就让按钮不见，读起来像是 agent 已经停了。
...
- **宿主侧不知道「当前选中的是哪个会话」。** 选中项是浏览器 `ClientSessions` 里按连接持久化的状态，没有任何宿主接口携带它，两个同时连着的浏览器还可以各选各的。宿主侧的分组因此一律按「无选中」求值——这对批量归档没有影响，规格本来就要求空会话**无条件跳过**，包括被选中的那个。浏览器侧不受此限：`ctx.sessions.list` 快照的 `current` 是公开的，归档区排除当前会话正是读它。
- **删除是单个会话粒度的。** 批量删除是逐个执行，其中一个失败不影响其余，每个会话各自报告结果。清账阶段失败时（工作区不放手、归档集合写不进去）报告说明**当前实际处于什么状态**，而不是谎报成功。
- **关闭不拦截宿主的 fail-loud。** DSH 把任何未捕获的 Promise 拒绝当成致命错误：写一行 `dsh: fatal load failure:` 然后退出进程。插件在关闭期间加了一道观测，抢在宿主的处理器之前打日志、点名当时正在拆哪个会话，**但不阻止退出**——吞掉宿主的 fail-loud 语义比进程退出更糟。
- **空白会话的标题栏上没有关闭按钮。** DSH 把空白会话渲染成 new-session 界面，根本没有标题栏，也就没有可供扩展的位置。空白会话也没有值得关的 agent。
...
pnpm --filter @dsh-plugins/session-archive test:e2e   # 两套都跑

node packages/session-archive/e2e/verify.mjs          # 归档区与侧栏；加 --headed 看着它跑
node packages/session-archive/e2e/verify-shutdown.mjs # 两个关闭入口
```
...
保留它的理由有两条。一是侧栏行内按钮依赖宿主未公开的 DOM 层级与 fiber 形状，而 kill-switch 的行为是**静默停用**：没有这套回归，下一次 DSH 升级会让这个功能悄无声息地消失。单测里那份 jsdom fixture 是照真实 DOM 量出来的，也只有它能告诉你 fixture 过期了。

二是关闭那套里最重要的断言压根不在界面上：**跑完之后 DSH 进程还活着**。宿主把任何未捕获的拒绝都当成致命错误并退出，这种失败模式不会在单测里出现，只能靠真的起一个进程、真的关掉一批会话、然后看它还在不在。fixture 里可以按需注册 disposer 会拒绝的假 agent，把本机没有模型凭证就走不到的失败分支补上。
